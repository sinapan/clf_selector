...
''' libraries for feature manupulation '''
from sklearn.decomposition import PCA

''' libraries for balancing your data '''
from imblearn.over_sampling import ADASYN #RandomOverSampler #SMOTE #ADASYN
#from imblearn.under_sampling import TomekLinks

''' libraries for predefined models '''
...
'''
# Import data
CSV_IN   = pd.read_csv('feature_sheet.csv',index_col=0)
# Input data - select column names to be used as features
X_mat_original = CSV_IN.loc[:,('width','height','age')]
# Input Labels - extract classication lables from the dataset
...
def rungraph(solver_method, NN_n, X_tr, X_ts, y_tr, y_ts):
    ...
    if   solver_method == 'NN_lbfgs_1':
        ...
        clf = MLPClassifier(hidden_layer_sizes=NN_n, activation='logistic', solver='sgd',   learning_rate='adaptive', max_iter=5000, shuffle=True, early_stopping=True)
    elif solver_method == 'NN_sgd_2':
        clf = MLPClassifier(hidden_layer_sizes=NN_n, activation='tanh',     solver='sgd',   learning_rate='adaptive', max_iter=5000, shuffle=True, early_stopping=True)
    elif solver_method == 'NN_sgd_3':
        clf = MLPClassifier(hidden_layer_sizes=NN_n, activation='relu',     solver='sgd',   learning_rate='adaptive', max_iter=5000, shuffle=True, early_stopping=True)
        ...
        clf = MLPClassifier(hidden_layer_sizes=NN_n, activation='relu',     solver='adam',  learning_rate='constant', max_iter=5000, shuffle=True, early_stopping=True, learning_rate_init=0.01)
...
#            pcaCounter = j
            ...
            # loop over different classification configuration
            ...
            ac_NN3_adam_3[i,j], temp   = rungraph(solver_method, (20,10,5),  X_train_pca, X_test_pca, y_train, y_test)
            ac_NN5_adam_3[i,j], temp   = rungraph(solver_method, (20,30,5),  X_train_pca, X_test_pca, y_train, y_test)
            ac_NN10_adam_3[i,j], temp  = rungraph(solver_method, (10,5,3), X_train_pca, X_test_pca, y_train, y_test)            
            ac_NN20_adam_3[i,j], temp  = rungraph(solver_method, (30,20,10), X_train_pca, X_test_pca, y_train, y_test)
